...
public sealed class DownloadPaymentFileHandler(
    ...
    TimeProvider time) : IQueryHandler<DownloadPaymentFileQuery, PaymentFile>
{
    public async Task<PaymentFile> HandleAsync(DownloadPaymentFileQuery query, CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(query);

        PaymentRun run = await db.PaymentRuns.AsNoTracking().SingleOrNotFoundAsync(query.PaymentRunId, cancellationToken);
