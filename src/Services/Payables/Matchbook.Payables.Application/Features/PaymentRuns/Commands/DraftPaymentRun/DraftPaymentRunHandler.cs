...
public sealed class DraftPaymentRunHandler(IPayablesDb db, TimeProvider time) : ICommandHandler<DraftPaymentRunCommand, PaymentRunView>
{
    public async Task<PaymentRunView> HandleAsync(DraftPaymentRunCommand command, CancellationToken cancellationToken)
    {
        ...
        PaymentRunDraft draft = PaymentRun.Draft(command.Id ?? Guid.CreateVersion7(now), executionDate, command.Treasurer, candidates, suppliers, now);
