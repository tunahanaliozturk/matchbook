...
public sealed class ReleasePaymentRunHandler(IPayablesDb db, IEventPublisher publisher, PayablesMetrics metrics, TimeProvider time)
    : ICommandHandler<ReleasePaymentRunCommand, PaymentRunView>
{
    public async Task<PaymentRunView> HandleAsync(ReleasePaymentRunCommand command, CancellationToken cancellationToken)
    {
        ...
        run.Release(command.Treasurer, suppliers, now);
