...
public sealed class CaptureInvoiceHandler(IPayablesDb db, InvoiceMatcher matcher, PayablesMetrics metrics, TimeProvider time)
    : ICommandHandler<CaptureInvoiceCommand, InvoiceView>
{
    public async Task<InvoiceView> HandleAsync(CaptureInvoiceCommand command, CancellationToken cancellationToken)
    {
