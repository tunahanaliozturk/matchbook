...
public sealed class FundsReservedHandler(IRequisitionsDb db, ILogger<FundsReservedHandler> logger)
    : IIntegrationEventHandler<FundsReserved>
{
    public async Task HandleAsync(FundsReserved integrationEvent, CancellationToken cancellationToken)
    {
        ...
        CostCentre costCentre = await db.CostCentres
            .AsNoTracking()
            .FirstOrDefaultAsync(centre => centre.Code == integrationEvent.CostCentreCode, cancellationToken)
            ?? throw new InvalidOperationException(
                $"Funds were reserved for requisition {integrationEvent.RequisitionId} on cost centre {integrationEvent.CostCentreCode}, which has no local copy.");

        await db.RecordAsync(
            integrationEvent.RequisitionId,
            nameof(FundsReserved),
            logger,
            requisition => requisition.RecordFundsReserved(costCentre.ManagerId, integrationEvent.OccurredAt),
            cancellationToken);
    }
}
