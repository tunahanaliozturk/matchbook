...
public sealed class ListMyApprovalsHandler(IRequisitionsDb db)
    : IQueryHandler<ListMyApprovalsQuery, Page<RequisitionSummary>>
{
    public async Task<Page<RequisitionSummary>> HandleAsync(ListMyApprovalsQuery query, CancellationToken cancellationToken)
    {
