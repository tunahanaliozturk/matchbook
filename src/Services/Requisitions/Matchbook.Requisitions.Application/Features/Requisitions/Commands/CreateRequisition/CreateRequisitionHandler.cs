...
public sealed class CreateRequisitionHandler(IRequisitionsDb db, TimeProvider time)
    : ICommandHandler<CreateRequisitionCommand, RequisitionView>
{
    ...
    public async Task<RequisitionView> HandleAsync(CreateRequisitionCommand command, CancellationToken cancellationToken)
    {
        (Guid? id, RequisitionDetails details, Actor actor) = command;
