...
public sealed class RequisitionSubmittedConsumer(RequisitionSubmittedHandler handler) : IConsumer<RequisitionSubmitted>
{
    public Task Consume(ConsumeContext<RequisitionSubmitted> context) =>
        handler.HandleAsync(context.Message, context.CancellationToken);
}

public sealed class RequisitionRejectedConsumer(RequisitionRejectedHandler handler) : IConsumer<RequisitionRejected>
{
    public Task Consume(ConsumeContext<RequisitionRejected> context) =>
        handler.HandleAsync(context.Message, context.CancellationToken);
}

public sealed class RequisitionCancelledConsumer(RequisitionCancelledHandler handler) : IConsumer<RequisitionCancelled>
{
    public Task Consume(ConsumeContext<RequisitionCancelled> context) =>
        handler.HandleAsync(context.Message, context.CancellationToken);
}

public sealed class PurchaseOrderCommitmentRequestedConsumer(PurchaseOrderCommitmentRequestedHandler handler)
    : IConsumer<PurchaseOrderCommitmentRequested>
{
