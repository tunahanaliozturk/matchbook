...
/// <summary>
/// <c>/purchase-orders</c>. Each endpoint turns HTTP into one handler call and back. Refusals are exceptions the
...
internal static class PurchaseOrderEndpoints
{
    ...
    private static async Task<Ok<PurchaseOrderPage>> ListAsync(
        PurchaseOrderStatus? status, Guid? after, int? limit, ListPurchaseOrdersHandler handler, CancellationToken cancellationToken) =>
        TypedResults.Ok(await handler.HandleAsync(new ListPurchaseOrders(status, after, limit ?? DefaultLimit), cancellationToken));

    private static async Task<Ok<PurchaseOrderView>> GetAsync(
        Guid id, GetPurchaseOrderHandler handler, CancellationToken cancellationToken) =>
    ...
    private static async Task<Ok<PurchaseOrderView>> GetForRequisitionAsync(
        Guid requisitionId, GetPurchaseOrderForRequisitionHandler handler, CancellationToken cancellationToken) =>
        TypedResults.Ok(await handler.HandleAsync(requisitionId, cancellationToken));
    ...
    private static async Task<Ok<PurchaseOrderView>> AmendLineAsync(
        ...
        AmendDraftLineHandler handler,
        CancellationToken cancellationToken)
    {
        // Validation has run: both values are present.
        var command = new AmendDraftLine(id, lineNumber, request.Quantity!.Value, request.UnitPrice!.Value);
        return TypedResults.Ok(await handler.HandleAsync(command, user.ToActor(), cancellationToken));
    }
    ...
    private static async Task<Accepted<PurchaseOrderView>> IssueAsync(
        Guid id, ClaimsPrincipal user, IssuePurchaseOrderHandler handler, CancellationToken cancellationToken) =>
        TypedResults.Accepted((string?)null, await handler.HandleAsync(id, user.ToActor(), cancellationToken));

    private static async Task<Ok<PurchaseOrderView>> ShortCloseAsync(
        Guid id, ClaimsPrincipal user, ShortClosePurchaseOrderHandler handler, CancellationToken cancellationToken) =>
    ...
    private static async Task<Ok<PurchaseOrderView>> CancelAsync(
        Guid id, ClaimsPrincipal user, CancelPurchaseOrderHandler handler, CancellationToken cancellationToken) =>
        TypedResults.Ok(await handler.HandleAsync(id, user.ToActor(), cancellationToken));
        ...
        CancellationToken cancellationToken)
    {
        ...
        var command = new RecordReceipt(
            id,
            request.Id,
            [.. request.Lines!.Select(static line => new LineQuantity(line.LineNumber!.Value, line.Quantity!.Value))]);

        GoodsReceiptView receipt = await handler.HandleAsync(command, user.ToActor(), cancellationToken);
    ...
    private static async Task<Ok<IReadOnlyList<GoodsReceiptView>>> ListReceiptsAsync(
        Guid id, ListReceiptsHandler handler, CancellationToken cancellationToken) =>
        TypedResults.Ok(await handler.HandleAsync(id, cancellationToken));

    private static async Task<Ok<GoodsReceiptView>> GetReceiptAsync(
        Guid id, Guid receiptId, GetReceiptHandler handler, CancellationToken cancellationToken) =>
        TypedResults.Ok(await handler.HandleAsync(id, receiptId, cancellationToken));
}
