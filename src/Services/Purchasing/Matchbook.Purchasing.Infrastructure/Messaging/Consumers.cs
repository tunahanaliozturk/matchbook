...
public sealed class RequisitionApprovedConsumer(IIntegrationEventHandler<RequisitionApproved> handler) : IConsumer<RequisitionApproved>
{
    public Task Consume(ConsumeContext<RequisitionApproved> context) =>
        handler.HandleAsync(context.Message, context.CancellationToken);
}

public sealed class FundsCommittedConsumer(IIntegrationEventHandler<FundsCommitted> handler) : IConsumer<FundsCommitted>
{
    public Task Consume(ConsumeContext<FundsCommitted> context) =>
        handler.HandleAsync(context.Message, context.CancellationToken);
}

public sealed class FundsCommitmentRejectedConsumer(IIntegrationEventHandler<FundsCommitmentRejected> handler)
    : IConsumer<FundsCommitmentRejected>
{
